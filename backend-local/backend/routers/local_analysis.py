...
def _integrate_llm_structure_into_kg(kg_result: Dict[str, Any], llm_analysis: Dict[str, Any], file_path: str, dataset_id: str = None) -> Dict[str, Any]:
    ...
    allowed_unit_lower = {key.lower(): value for key, value in allowed_unit_map.items()}

    def normalize_unit(raw_unit: Any, level: int) -> str:
        if isinstance(raw_unit, str):
            trimmed = raw_unit.strip()
            if trimmed in allowed_unit_map:
                return allowed_unit_map[trimmed]
            lowered = trimmed.lower()
            if lowered in allowed_unit_lower:
                return allowed_unit_lower[lowered]
    ...
    def normalize_number(raw_number: Any, fallback_index: int) -> str:
        if isinstance(raw_number, (int, float)):
            digits = re.findall(r"\d+", str(raw_number))
    ...
    def process_structures(
        ...
        for idx, node in enumerate(nodes):
            ...
            normalized_unit = normalize_unit(node.get("unit"), level)
            normalized_number = normalize_number(node.get("number"), idx)
...
async def generate_knowledge_graph(
    ...
    try:
        ...
        if use_llm and structure_results.get("llm_analysis"):
            ...
            # LLM 분석 자체가 실패한 경우 - 전체 실패로 처리
            import logging
            logger = logging.getLogger(__name__)
            logger.error("❌ LLM 구조 분석 실패 - Knowledge Graph 생성 중단")

            error_msg = structure_results.get("llm_error", "LLM 구조 분석 실패")
            raise HTTPException(
                status_code=500,
                detail=f"LLM 기반 Knowledge Graph 생성 실패: {error_msg}"
            )
        ...
        if use_llm and isinstance(kg_result, dict):
            try:
                ...
                if False:  # memgraph_service.is_connected():
