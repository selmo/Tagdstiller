"""
Knowledge Graph JSON을 Cypher 쿼리로 변환하는 유틸리티

JSON 형식의 Knowledge Graph를 Neo4j/Memgraph용 Cypher 쿼리로 변환합니다.
"""
...
class KGToCypherConverter:
    """Knowledge Graph JSON → Cypher 변환기"""

    def __init__(self):
        self.node_id_to_var = {}  # 노드 ID를 Cypher 변수명으로 매핑
        self.created_nodes = set()  # 생성된 노드 추적
    ...
    def convert_file(
        ...
        """JSON 파일을 읽어 Cypher로 변환

        Args:
            json_file_path: 입력 JSON 파일 경로
            output_file_path: 출력 Cypher 파일 경로 (None이면 자동 생성)
            clear_db: 변환 전 데이터베이스 초기화 여부
            create_indexes: 인덱스 생성 쿼리 포함 여부
        ...
        print(f"✅ Cypher 파일 생성 완료: {output_file_path}")
        return cypher
    ...
    def convert(
        ...
        """Knowledge Graph JSON을 Cypher 쿼리로 변환

        Args:
            kg_data: Knowledge Graph JSON 데이터
            clear_db: 데이터베이스 초기화 쿼리 포함 여부
            create_indexes: 인덱스 생성 쿼리 포함 여부
...
def convert_kg_json_to_cypher(
    ...
    """Knowledge Graph JSON 파일을 Cypher로 변환하는 편의 함수

    Args:
        json_file: 입력 JSON 파일 경로
        output_file: 출력 Cypher 파일 경로 (None이면 자동)
        clear_db: 데이터베이스 초기화 포함 여부
        create_indexes: 인덱스 생성 포함 여부
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or '--help' in sys.argv or '-h' in sys.argv:
        ...
        print()
        print("Arguments:")
        print("  json_file       입력 Knowledge Graph JSON 파일")
        print("  output_file     출력 Cypher 파일 (선택사항, 기본값: <json_file>.cypher)")
        print()
        print("Options:")
        print("  --clear-db      데이터베이스 초기화 쿼리 포함 (MATCH (n) DETACH DELETE n)")
        print("  --no-indexes    인덱스 생성 쿼리 제외")
        print("  -h, --help      이 도움말 표시")
        print()
        print("Examples:")
    ...
    print(f"🗑️  DB 초기화: {clear_db}")
    print(f"📊 인덱스 생성: {create_indexes}")
    print()
    ...
    try:
        ...
        print(f"   - 총 라인: {len(cypher.splitlines())}줄")
