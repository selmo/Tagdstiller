...
Output:"""
    ...
    def _call_ollama_fallback(self, prompt: str) -> tuple[str, dict, dict]:
        ...
        try:
            ...
            stream_enabled = (self.config or {}).get('stream', True)  # 기본적으로 스트림 활성화
            ...
            logger.info(f"🚀 Ollama API 폴백 호출 시작 (Stream: {stream_enabled}) - 모델: {self.model_name}")

            timeout = self.config.get('timeout', 30) if self.config else 30

            if stream_enabled:
                return self._ollama_stream_request(payload, timeout, logger, debug_logger, llm_params, prompt)
            else:
                return self._ollama_non_stream_request(payload, timeout, logger, debug_logger, llm_params, prompt)
    ...
    def _ollama_stream_request(self, payload: dict, timeout: int, logger, debug_logger, llm_params: dict, prompt: str) -> tuple[str, dict, dict]:
        """Ollama 스트림 요청 처리"""
        import json

        logger.info("🌊 Ollama Stream 폴백 모드로 호출 시작")
        collected_response = []
        chunk_count = 0
        ...
        try:
            with requests.post(
                ...
                if response.status_code != 200:
                    logger.error(f"❌ Ollama Stream 폴백 오류: {response.status_code} - {response.text}")
                        ...
                        try:
                            ...
                            if chunk_text:
                                ...
                                if chunk_count % 20 == 0:
                                    ...
                                    logger.info(f"📝 Ollama Stream 진행 중: {chunk_count}개 청크, {len(total_text)}자 수신")
                                    logger.debug(f"현재까지 내용 미리보기: {total_text[:200]}...")

                            # 완료 체크
                            if chunk_data.get("done", False):
                                logger.info("🏁 Ollama Stream 완료")
                                break

                        except json.JSONDecodeError as e:
                            logger.warning(f"⚠️ Ollama Stream 청크 파싱 실패: {e}")
                            continue

                final_response = ''.join(collected_response)
                logger.info(f"✅ Ollama Stream 폴백 완료: {chunk_count}개 청크, 총 {len(final_response)}자")

                # 디버그 로깅
                debug_logger.log_algorithm_application(
                    extractor_name="llm",
                    algorithm=f"{self.provider}_stream_fallback_generation",
                    input_candidates=[("prompt", len(prompt))],
                    output_keywords=[("response", len(final_response))],
                    algorithm_params={
                        "provider": self.provider,
                        "model": self.model_name,
                        "stream_mode": True,
                ...
                return final_response.strip(), payload, {"response": final_response, "done": True, "stream_chunks": chunk_count}

        except Exception as e:
            logger.error(f"❌ Ollama Stream 폴백 처리 실패: {e}")
            return "", payload, {"error": str(e)}

    def _ollama_non_stream_request(self, payload: dict, timeout: int, logger, debug_logger, llm_params: dict, prompt: str) -> tuple[str, dict, dict]:
        """Ollama 비스트림 요청 처리"""
        try:
            ...
            if response.status_code == 200:
                ...
                debug_logger.log_algorithm_application(
                    ...
                    input_candidates=[("prompt", len(prompt))],
                    output_keywords=[("response", len(result))],
                    algorithm_params={
                        "provider": self.provider,
                        "model": self.model_name,
                        "fallback_mode": True,
    ...
    def _call_gemini(self, prompt: str) -> str:
        """Google Generative Language (Gemini) REST 호출 (Stream 지원)."""
        import os
        import logging
        logger = logging.getLogger(__name__)
        ...
        try:
            if stream_enabled:
                return self._call_gemini_stream(url, payload, logger)
            else:
                return self._call_gemini_non_stream(url, payload, logger)
        except Exception as e:
            logger.error(f"❌ Gemini API 호출 실패: {e}")
            return ""

    def _call_gemini_stream(self, url: str, payload: dict, logger) -> str:
        """Gemini 스트림 응답 처리"""
        import json

        logger.info("🌊 Gemini Stream 모드로 호출 시작")
        ...
        timeout = (self.config or {}).get('timeout', 120)
        collected_response = []

        try:
            with requests.post(url, json=payload, headers=headers, timeout=timeout, stream=True) as response:
                if response.status_code != 200:
                    logger.error(f"❌ Gemini Stream 오류: {response.status_code} - {response.text}")
                    return ""
                    ...
                    if line:
                        ...
                        if line_str.startswith('data: '):
                            ...
                            try:
                                ...
                                logger.warning(f"⚠️ Gemini Stream 청크 파싱 실패: {e}")
                                continue

                final_response = ''.join(collected_response)
                logger.info(f"✅ Gemini Stream 완료: {chunk_count}개 청크, 총 {len(final_response)}자")
                return final_response

        except Exception as e:
            logger.error(f"❌ Gemini Stream 처리 실패: {e}")
            return ""

    def _call_gemini_non_stream(self, url: str, payload: dict, logger) -> str:
        """Gemini 비스트림 응답 처리"""
        timeout = (self.config or {}).get('timeout', 120)
        ...
        try:
            ...
            if r.status_code != 200:
                logger.error(f"❌ Gemini API 오류: {r.status_code} - {r.text}")
                return ""
            ...
            logger.info(f"✅ Gemini 비스트림 완료: {len(response)}자")
            return response

        except Exception as e:
            logger.error(f"❌ Gemini 비스트림 호출 실패: {e}")
            return ""
