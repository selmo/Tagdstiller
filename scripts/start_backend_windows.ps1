...
$envExists = $envList | Select-String "^$($env:CONDA_ENV)\s"
if (-not $envExists) {
...
if (-not (Test-Path "data\db.sqlite3")) {
    ...
    $initCommand = "import os; os.environ['OFFLINE_MODE']='true'; os.environ['SKIP_EXTERNAL_CHECKS']='true'; from main import app; print('Database initialized')"
    
    try {
        $result = & python -c $initCommand 2>&1
        ...
        $manualInit = "from db.db import Base, engine; Base.metadata.create_all(bind=engine); print('Manual DB init complete')"
        & python -c $manualInit
...
try {
    ...
    if ($portInUse) {
        ...
        Write-Host "다른 포트를 사용하려면: `$env:PORT=8001 후 다시 실행" -ForegroundColor Yellow
    ...
    # Get-NetTCPConnection이 없는 경우 무시
    Write-Host "포트 확인 건너뜀" -ForegroundColor Yellow
}
...
# Conda 환경 활성화를 위한 환경 설정
$condaPath = (Get-Command conda).Source
$condaBase = Split-Path (Split-Path $condaPath)
$condaScript = Join-Path $condaBase "etc\profile.d\conda.sh"

# 서버 실행
if ($Mode -eq "prod") {
    Write-Host "🏭 프로덕션 모드로 시작" -ForegroundColor Green
    & conda run -n $env:CONDA_ENV uvicorn main:app --host $env:HOST --port $env:PORT
} else {
    Write-Host "🔧 개발 모드로 시작 (자동 리로드)" -ForegroundColor Green
    & conda run -n $env:CONDA_ENV uvicorn main:app --reload --host $env:HOST --port $env:PORT
}